...
class DiffusionModel:
    ...
    def get_pipe(cls) -> StableDiffusionXLPipeline:
        if cls._pipe is None:
            logger.info("Loading SDXL-Turbo model with fixed VAE: %s", MODEL_ID)

            # Load the "fixed" fp16-compatible VAE
            vae = AutoencoderKL.from_pretrained(
                "madebyollin/sdxl-vae-fp16-fix",
                torch_dtype=torch.float16
            )

            # Load main pipeline with that VAE
            ...
            try:
                ...
                logger.debug("Slicing/tiling not available: %s", e)
    ...
    def _warmup(cls):
        try:
            logger.info("Warming up with prompt='%s', steps=%d", WARMUP_PROMPT, WARMUP_STEPS)
    ...
    try:
        with torch.inference_mode():
            if USE_AUTOCAST and device == "cuda":
                logger.info("Using autocast: UNet/text FP16, VAE FP32")
                ...
                # restore old VAE dtype
                pipe.vae.to(vae_orig_dtype)
            else:
                logger.info("Autocast disabled or not CUDA: running full precision")
                result = pipe(
                    prompt=prompt,
                    num_inference_steps=steps,
                    width=width,
...
def generate_from_prompts(prompts: List[str], steps: int = 8, width: int = 1024, height: int = 1024, fmt: str = "JPEG") -> List[bytes]:
    if not prompts:
        return []
    if len(prompts) == 1:
        return [_generate_raw(prompts[0], steps, width, height, fmt)]

    if len(prompts) > MAX_BATCH:
        raise ValueError(f"Batch size {len(prompts)} exceeds MAX_BATCH={MAX_BATCH}")

    pipe = DiffusionModel.get_pipe()
    logger.info("Batch generation size=%d | autocast=%s | device=%s", len(prompts), USE_AUTOCAST, DEVICE)
